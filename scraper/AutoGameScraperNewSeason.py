...
CUTOFF_DATE = datetime.strptime("2025-08-01", "%Y-%m-%d").date()

# ✅ Filter function
def filter_games_after_cutoff(games):
    """Return only games played on or after the cutoff date."""
    filtered = [g for g in games if g["game_date"] is not None and g["game_date"] >= CUTOFF_DATE]
    print(f"Filtered out {len(games) - len(filtered)} games older than {CUTOFF_DATE}")
    return filtered
...
def fetch_players_from_db():
    cursor.execute("SELECT player_id, player_link FROM players")
...
def insert_games_into_postgres(data, player_id):
    """Insert only new games if there are more new than existing (after cutoff date)."""
    cursor.execute(
        "SELECT COUNT(*) FROM games WHERE player_id = %s AND game_date >= %s",
        (player_id, CUTOFF_DATE)
    )
    existing_count = cursor.fetchone()[0]

    print(f"Existing records for player '{player_id}' after {CUTOFF_DATE}: {existing_count}")
    new_count = len(data)
    print(f"Scraped new entries for player '{player_id}' after cutoff: {new_count}")
...
def scrap_player(player):
    ...
    # ✅ Filter by cutoff before inserting
    filtered_games = filter_games_after_cutoff(games)
    insert_games_into_postgres(filtered_games, player["id"])
